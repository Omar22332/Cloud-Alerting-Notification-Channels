...
interface AddChannelModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (channel: Omit<NotificationChannel, 'id' | 'status'>) => void;
  initialData: Omit<NotificationChannel, 'id' | 'status'> | null;
}
...
export const AddChannelModal: React.FC<AddChannelModalProps> = ({ isOpen, onClose, onSave, initialData }) => {
  ...
  useEffect(() => {
    if (isOpen) {
        if (initialData) {
            ...
            setLabels(initialData.labels.length > 0 ? [...initialData.labels] : [{ key: '', value: ''}]);
  ...
  const handleSave = () => {
    ...
    onSave({ displayName, type, labels: filteredLabels, enabled });
    onClose();
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className="p-6">
          <h2 id="modal-title" className="text-xl font-semibold text-gray-900 dark:text-white">Add Notification Channel</h2>
